...
# Démarrer le conteneur Stable Diffusion en mode web avec le build créer

echo "Lancement du conteneur $CONTAINER_NAME en mode web"
podman run -dt --pod $POD_NAME --name $CONTAINER_NAME \
  -v "/mnt/podman/shared_volumes/models:/workspace/stable-diffusion-webui/models:Z" \
  -v "/mnt/podman/shared_volumes/images/stable-diffusion:/workspace/stable-diffusion-webui/outputs:Z" \
  -u $USER_UID:$USER_GID \
  --group-add=users \
  --workdir /workspace/stable-diffusion-webui \
  stable-diffusion-custom \
  /bin/bash -c "python3 launch.py --listen --xformers --enable-insecure-extension-access"
